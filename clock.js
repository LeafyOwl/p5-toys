...
function drawBird(y) {
  ...
  strokeWeight(6);
  line(0, -30, cos(hourAngle) * 35, sin(hourAngle) * 35 - 30); // Hour hand
...
// Draw the light blue semi-circular bird at the bottom left
function drawSwingingBird(x, y, angle) {
  push();
  translate(x, y); // Move to bird's position
  rotate(angle); // Rotate for the swinging motion

  // Draw bird's body (semi-circle)
  fill("#87CEEB"); // Light blue color for the body
  arc(0, 0, 50, 60, 0, 180); // Semi-circle body facing to the right
  circle(12.5, 0, 25);
  // Draw bird's eye
  fill("yellow"); // White for the eye
  ellipse(14, -3, 10, 10); // Eye positioned on the right side of the semi-circle
  fill(0, 0, 0); // White for the eye
  ellipse(14, -3, 6, 6);
  line(50, 60, 70, 70);

  // Draw bird's beak
  fill(255, 165, 0); // Orange color for the beak
  triangle(35, 0, 22, 5, 22, -9); // Small triangle for the beak
